...
export function setupImportHandler(cachedNotes) {
    ...
    importFile.addEventListener("change", async (e) => {
        ...
        try {
            ...
            alert("Couldn't read that file — is it a valid O(note) backup?");
        } finally {
            importFile.value = "";
...
async function validateBackup(parsed) {
    ...
    if(parsed.format !== "O(note)-backup") return { valid: false, reason: "This is not a valid O(note) backup file " };
